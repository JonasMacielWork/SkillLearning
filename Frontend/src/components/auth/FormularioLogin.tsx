    ...
    <Card className="glass-card border-glass-border p-6 max-w-md w-full mx-auto">
      ...
      <form onSubmit={aoEnviar} className="space-y-4 mt-4">
        <div>
          ...
          <Input type="text" value={usuario} onChange={(e) => setUsuario(e.target.value)} required className="glass-card border-glass-border" />
        </div>
        <div>
        ...
        <Button type="submit" disabled={carregando} className="glass-button w-full">{carregando ? "Entrando..." : "Entrar"}</Button>
      </form>
